# R1 (fade buyer dominance) — Checkpoint Review

> **Template — copy and fill at each R1 checkpoint transition.**
> Save filled copy as `docs/reviews/r1_n<level>_<UTC>.md`.
...
| live hit_rate | <FILL> |
| live mean_r | <FILL> |
| live retention vs 1.18 | <FILL> |
| historical n | <FILL> |
| historical mean_r | <FILL> |
| historical retention | 0.07 (known weak) |
...
## Per-fire detail (all live R1 fires this checkpoint)

| fire (NY) | session | entry | stop | target | MFE | MAE | final R | outcome |
|---|---|---|---|---|---|---|---|---|
| | | | | | | | | |

(One row per live fire. Pull from dashboard's Live Signal Trade Details card.)
...
☐ direction sign correct (R1 = short)
...
```
☐ Late fills — does R1 fire near intra-bar peaks where reversal is exhausted?
☐ Trend regime — does R1 lose during persistent up-trends?
☐ News reaction — clustering near 8:30 NY ET data prints?
☐ Direction sign — buyer-down ≠ short bias inversion bug?
☐ Threshold over-fit — RULE_DELTA_DOMINANCE_REAL=0.04 too tight on live?
☐ Volume reconciliation — do losses align with high vol_match mismatch bars?
```

R1 historical retention is 0.07. Even an OK verdict at n=30 may not be tradable
after costs/slippage. Default skepticism is high.
...
Did the new fires (between checkpoints) trend with or against the running
mean? Convergent → trust. Divergent → noise; weight prior data carefully.
...
| ☐ INVESTIGATE | ambiguous — propose Phase 2C iteration (sign-flip, etc.) |
...
n=10 / n=15:
  ...
  ☐ Do NOT change rules
  ☐ Do NOT pause R1 production firing
  ☐ Do NOT auto-revert

n=30:
  ☐ Do NOT auto-revert (env flag flip is manual)
  ☐ Do NOT change R1 thresholds individually (revert is all-or-nothing)
...
|---|---|---|
| n=10 OK | n=15 in ~2 days at current rate | re-review |
| n=10 WARN | n=15 in ~2 days | investigate root cause now, document |
| n=15 OK convergent | n=30 in ~5 days | re-review, prepare verdict template |
