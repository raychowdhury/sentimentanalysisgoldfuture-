...
def compute(signal: str, gold_ind: dict | None) -> dict:
    """Compute trade setup for the given signal and gold indicators."""
    if gold_ind is None:
        return _no_trade(None, "Gold market data unavailable")

    entry = gold_ind["current"]

    if signal in ("STRONG_BUY", "BUY"):
        return _buy(entry, gold_ind)
    if signal in ("STRONG_SELL", "SELL"):
        return _sell(entry, gold_ind)

    return _no_trade(entry, f"Signal is {signal} — no directional setup")


# ── BUY setup ─────────────────────────────────────────────────────────────────

def _buy(entry: float, ind: dict) -> dict:
    """
    Entry  : current gold price
    Stop   : below min(EMA20, 14d_low) with STOP_BUFFER_PCT cushion
    Target : entry + MIN_RR * risk  (ensures >= 1:2 RR by construction)
    """
    invalidation = min(ind["ema20"], ind["recent_low_14d"])
    stop = invalidation * (1.0 - config.STOP_BUFFER_PCT)
    risk = entry - stop

    if risk <= 0:
        return _no_trade(entry, "Entry is at or below the invalidation level — BUY not valid")
    ...
    return _fmt(entry, stop, tp, risk)


# ── SELL setup ────────────────────────────────────────────────────────────────

def _sell(entry: float, ind: dict) -> dict:
    """
    Entry  : current gold price
    Stop   : above max(EMA20, 14d_high) with STOP_BUFFER_PCT cushion
    Target : entry - MIN_RR * risk  (ensures >= 1:2 RR by construction)
    """
    ...
    if risk <= 0:
        return _no_trade(entry, "Entry is at or above the invalidation level — SELL not valid")
...
def _fmt(entry: float, stop: float, tp: float, risk: float) -> dict:
    reward = abs(tp - entry)
    rr     = reward / risk
    ...
    return {
        ...
        "minimum_required_rr": config.MIN_RR,
        "trade_valid":         valid,
        "setup_note":          None,
    }


def _no_trade(entry: float | None, reason: str) -> dict:
