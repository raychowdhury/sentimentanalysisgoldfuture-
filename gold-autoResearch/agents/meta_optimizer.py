...
suggestion back into program.md under `## Next Experiment`, and appends a
timestamped record to run_log.md.

Uses prompt caching on the system prompt so subsequent calls with the same
research instructions are cheaper (the run log content is dynamic and stays
uncached on purpose).
"""
...
SYSTEM_PROMPT = (
    "You are a quantitative research assistant reviewing gold futures model "
    "experiments. Analyze the run log and identify: what feature or "
    "hyperparameter change had the biggest positive impact? What should the "
    "next experiment be? Respond in JSON: "
...
def _client() -> Any:
    """Lazy-import so the rest of the package works without anthropic installed."""
    try:
        import anthropic
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "anthropic SDK is required for meta_optimizer — "
